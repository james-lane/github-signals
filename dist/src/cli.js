...
class App {
    ...
    commitsView() {
        ...
        if (!rows.length)
            this.line(yellow(`No cached commits. Press r to refresh the last ${this.config.commitLedgerDays} day${this.config.commitLedgerDays === 1 ? '' : 's'}.`));
    }
    ...
    async clearDatabaseSection() {
        ...
        if (section === 'commits' || section === 'all')
            Object.assign(this.commitLedger, { commits: [], focusedCommits: null, repositoryFilter: '', repositories: 0, errors: [], page: 0, selection: 0 });
