...
export async function recordSnapshot(config, data, cwd = process.cwd()) {
    if (!data || data.engineers?.some(item => item.error) || data.repositories?.some(item => item.error))
        return false;
    ...
    try {
        ...
        if (last && new Date(data.fetchedAt).getTime() - new Date(last.captured_at).getTime() < 15 * 60_000)
            return false;
    ...
    try {
      ...
      ON CONFLICT(hostname, organization, repository, sha) DO UPDATE SET
        ...
        for (const commit of commits || [])
            insert.run(config.hostname, commit.organization, commit.repository, commit.sha, commit.branch, commit.author, commit.committedAt, commit.message, commit.url);
...
export function clearStoredData(section, cwd = process.cwd()) {
    ...
    if (!allowed.has(section))
        throw new Error('Unknown database section.');
    const db = openHistory(cwd);
    try {
        db.exec('BEGIN');
        ...
        if (section === 'ci' || section === 'all')
            db.exec('DELETE FROM ci_runs');
        if (section === 'commits' || section === 'all')
            db.exec('DELETE FROM organization_commits');
        db.exec('COMMIT');
    }
    catch (error) {
        try {
