...
function openHistory(cwd = process.cwd()) {
  ...
  if (!ciColumns.some(column => column.name === 'workflow_path')) db.exec('ALTER TABLE ci_runs ADD COLUMN workflow_path TEXT');
...
export async function recordCiRuns(config, runs, cwd = process.cwd()) {
  ...
  try {
    ...
    for (const run of runs) {
      ...
      insert.run(run.repository, run.id, run.attempt, run.workflowId, run.workflowPath, run.workflow, run.title, run.event, run.status, run.conclusion,
