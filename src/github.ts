...
export function normalizeOrganizationCommit(item, repository) {
  return {
    sha: String(item?.sha || ''),
...
export async function fetchOrganizationCommits(config, onProgress = () => {}, { signal } = {}) {
  if (!config.organization) return { commits: [], repositories: 0, errors: [] };
  const repositories = [];
  ...
  for (let offset = 0; offset < repositories.length; offset += concurrency) {
    await Promise.all(repositories.slice(offset, offset + concurrency).map(async repository => {
      try {
        ...
        errors.push({ repository: repository.full_name, error: error.message });
      } finally {
        completed += 1;
    ...
    try {
      ...
      chunk.forEach((loginName, index) => {
        ...
        const addContributions = (groups, key) => groups.forEach(group => {
          ...
          if (!repositoryActivity.has(name)) repositoryActivity.set(name, { name, commits: 0, pullRequests: 0, merged: 0, reviews: 0, activeDays: 0 });
...
export function engineerSignalsFromRepositories(engineers, activity, since) {
  ...
  const repositorySignal = (engineer, repository) => {
    if (!engineer.repositoryActivity.has(repository)) engineer.repositoryActivity.set(repository, {
  ...
  const markActive = (focus, date) => { if (date) focus.activeDates.add(date.slice(0, 10)); };
