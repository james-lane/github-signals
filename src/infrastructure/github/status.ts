import type { GitHubStatus } from '../../domain/models.js';
...
const indicators = new Set<GitHubStatus['indicator']>(['none', 'minor', 'major', 'critical', 'maintenance']);
...
export function parseGitHubStatus(payload: unknown): GitHubStatus {
  ...
  if (typeof indicator !== 'string' || !indicators.has(indicator as GitHubStatus['indicator']) || typeof description !== 'string' || !description.trim()) {
    throw new Error('GitHub Status returned an unexpected response.');
  }
  return { indicator: indicator as GitHubStatus['indicator'], description: description.trim().slice(0, 80), checkedAt: new Date().toISOString() };
}
